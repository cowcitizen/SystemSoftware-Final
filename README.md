...
본 실습에서 학생이 구현해야 할 사항은 크게 두 영역으로 구성된다.
...
아래는 `src/trainer.cpp` 중에서 학생이 직접 구현해야 하는 세 함수의 스켈레톤이다.
...
빌드/실행/진행률 출력 로직은 이미 포함되어 있으며, 학생은 **요약 통계 출력 부분**을 완성해야 한다.
...
**프로세스 생성, 파이프 연결, 시그널 처리, 쉘 스크립트에 의한 오케스트레이션**을 실제 코드로 구현하는 데 있다.
제공된 구조와 UML 다이어그램을 참조하여, 지정된 TODO 부분을 중심으로 구현을 완료하기 바란다.
